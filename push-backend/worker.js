...
async function googleCerts() {
  ...
  if (!r.ok) throw new Error("firebase_certs");
  const keys = await r.json();
  const cc = r.headers.get("cache-control") || "";
  const m = cc.match(/max-age=(\d+)/);
  certCache = { keys, expires: Date.now() + (m ? Number(m[1]) : 1800) * 1000 };
  return keys;
}
...
async function verifyFirebaseIdToken(token) {
  ...
  if (payload.aud !== PROJECT_ID || payload.iss !== `https://securetoken.google.com/${PROJECT_ID}` || !payload.sub || payload.exp <= now || payload.iat > now + 60) {
    throw new Error("token_claims");
  }
  const certs = await googleCerts();
  const pem = certs[header.kid];
  if (!pem) throw new Error("token_kid");
  const der = b64urlToBytes(pem.replace(/-----BEGIN CERTIFICATE-----|-----END CERTIFICATE-----|\s/g, "").replace(/\+/g,"-").replace(/\//g,"_"));
...
function docData(doc) {
  return Object.fromEntries(Object.entries((doc && doc.fields) || {}).map(([k,v]) => [k, fv(v)]));
}
...
async function sendOneSignal(env, uids, title, message, url, data) {
...
function participantesPartida(p) {
  return [...new Set([...(Array.isArray(p.jugadores) ? p.jugadores : []), ...(Array.isArray(p.reservas) ? p.reservas : [])].filter(Boolean))];
}
...
async function handleChat(env, body, token, uid) {
  ...
  if (tipo === "partida") {
    ...
    return sendOneSignal(env, miembros.filter(x => x !== uid), `Mensaje de ${msg.n || "un jugador"}`, msg.t || "Nuevo mensaje en una partida", `https://padelplayers.github.io/p-del-app/?chat=partida&partidaId=${encodeURIComponent(chatId)}`, { tipo: "chat_partida", chatId, messageId });
  }
  ...
  return sendOneSignal(env, participantes.filter(x => x !== uid), `Mensaje privado de ${msg.n || "un jugador"}`, msg.t || "Tienes un nuevo mensaje privado", `https://padelplayers.github.io/p-del-app/?chat=privado&chatId=${encodeURIComponent(chatId)}`, { tipo: "chat_privado", chatId, messageId });
}
...
async function handleNotification(env, body, token, uid) {
  ...
  return sendOneSignal(env, [n.uid], n.titulo || "Pádel Players Morvedre", n.mensaje || "Tienes un nuevo aviso de partida", `https://padelplayers.github.io/p-del-app/?partidaId=${encodeURIComponent(n.partidaId)}`, { tipo: n.tipo || "aviso", partidaId: n.partidaId, notificationId: id });
}
